...
function jobSequence() {
  ...
  let maxProfit=0;
  for (const job of jobs) {
    if (minJobs.length===0||job.deadline>minJobs[minJobs.length-1].deadline) {
        minJobs.push(job);
...
console.log(`the maximum profit is ${result.maxProfit} by doing minimum job: ${result.minJobs.map((job)=>job.id)} jobs`);